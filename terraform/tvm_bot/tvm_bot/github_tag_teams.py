...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--team-issue", default="10317", help="issue number to look at for ccs"
    )
    parser.add_argument(
        "--team-issue-json",
        help="(testing only) issue JSON to parse rather than fetch from GitHub",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    if (issue is None and pr is None) or (issue is not None and pr is not None):
        raise RuntimeError(
            "Exactly one of $PR or $ISSUE must be set in the environment"
        )
    ...
    if issue is not None:
        ...
        raise RuntimeError(
            "Unreachable, please report a bug with a link to the failed workflow"
        )
